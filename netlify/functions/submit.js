...
exports.handler = async (event) => {
  try {
    ...
    if (RECAPTCHA_SECRET && data.recaptcha_token) {
      const verifyResp = await fetch("https://www.google.com/recaptcha/api/siteverify", {
        ...
        body: new URLSearchParams({
          ...
          // remoteip opcional: Netlify pasa IP en x-forwarded-for
    ...
    // 2) Reenviar a Google Apps Script (tu Sheet)
    if (!APPS_SCRIPT_ENDPOINT) {
      console.error("APPS_SCRIPT_ENDPOINT no configurado");
      alert( "APPS_SCRIPT_ENDPOINT no configurado" );
    } else {
      // Reenviamos exactamente los mismos campos del form
      const forwardBody = new URLSearchParams(data).toString();
      ...
      if (!res.ok) {
        ...
        console.error("Apps Script respondió error:", res.status, txt);
        // No rompemos UX aunque falle; Mailchimp igual recibió por action
      }
    }

    return { statusCode: 200, body: "OK" };
  } catch (err) {
    console.error("Function /api/submit error:", err);
    return { statusCode: 500, body: "Server error" };
  }
};
